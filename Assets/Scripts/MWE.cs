...
public class MWE : MonoBehaviour
{
...
/// <summary>
 ...
 public class WeightedMultiGraph<NodeLabel, EdgeLabel> {
    ...
    void Start()
    {
        ...
        // Graph covering escaping level 1
        // Each vertex is a given room that a player can be in
        // Each edge indicates the probability that a player will perform a given action

        var spawnRoom = G.addVertex("Spawn Room", true); // Starting State
        var keyRoom = G.addVertex("Key Room");
        var goblinRoom1 = G.addVertex("Goblin Room 1");
        var goblinRoom2 = G.addVertex("Goblin Room 2");
        var escapeRoom = G.addVertex("Escape Room");
        var escape = G.addVertex("Escape!", false, true); // Acceptance State
        
        G.addEdge(spawnRoom, keyRoom, "Spawn to Key", 0.45, 1.0);
        G.addEdge(spawnRoom, goblinRoom1, "Spawn to Goblin Room 1", 0.45, 1.0);
        G.addEdge(spawnRoom, spawnRoom, "Stay in Spawn", 0.1, 0.0);

        G.addEdge(keyRoom, spawnRoom, "Key to Spawn", 0.1, -1.0);
        G.addEdge(keyRoom, goblinRoom2, "Key to Goblin Room 2", 0.8, 1.0);
        G.addEdge(keyRoom, keyRoom, "Stay in Key", 0.1, 0.0);

        G.addEdge(goblinRoom1, spawnRoom, "Goblin Room 1 to Spawn", 0.1, -1.0);
        G.addEdge(goblinRoom1, goblinRoom2, "Goblin Room 1 to Goblin Room 2", 0.8, 1.0);
        G.addEdge(goblinRoom1, goblinRoom1, "Stay in Goblin Room 1", 0.1, 0.0);

        G.addEdge(goblinRoom2, keyRoom, "Goblin Room 2 to Key", 0.1, -1.0);
        G.addEdge(goblinRoom2, goblinRoom1, "Goblin Room 2 to Goblin Room 1", 0.1, -1.0);
        G.addEdge(goblinRoom2, escapeRoom, "Goblin Room 2 to Escape Room", 0.7, 5.0);
        G.addEdge(goblinRoom2, goblinRoom2, "Stay in Goblin Room 2", 0.1, 0.0);

        G.addEdge(escapeRoom, goblinRoom2, "Escape Room to Goblin Room 2", 0.05, -1.0);
        G.addEdge(escapeRoom, escape, "Escape Room to Escape!", 0.90, 10.0);
        G.addEdge(escapeRoom, escapeRoom, "Stay in Escape Room", 0.05, 0.0);
